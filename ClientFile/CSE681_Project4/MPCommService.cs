...
namespace MessagePassingComm
{
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName,string from,string to)
        {
            ...
            try
            {
                string path = Path.Combine(from, fileName);
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName,to);
                while (true)
                {
    ...
    class TestPCommService
    {
        ...
        static bool compareFileBytes(string filename)
        {
            ...
            string fileSpec2 = Path.Combine(RepoEnvironment.root, filename);
            try
            {
